          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                value={newItem.customerContact}
                onChange={handleInputChange}
                placeholder="Např. 777 123 456"
                className={inputClass}
              / autoComplete="tel" inputMode="tel" pattern="[\d\s+]*" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Telefonní kontakt 2</label>
              <input type="text" name="customerPhone2" value={newItem.customerPhone2} onChange={handleInputChange} placeholder="Např. 606 123 456" className={inputClass} value={newItem.customerContact} onChange={handleInputChange} placeholder="Např. 777 123 456" className={inputClass} / autoComplete="tel-secondary" inputMode="tel" pattern="[\d\s+]*" />
            </div>
          </div>
  ...
  const DeliveryFilterView = ({ transactions, drivers }) => {
    ...
    const handlePrint = () => {
      ...
      const rows = filteredDeliveries.map((d, idx) => {
        ...
        const telHtml = tel2 ? `${tel1}<br>${tel2}` : tel1;
